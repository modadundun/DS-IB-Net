...
def train(epochs, train_loader, all_test_loader, args, model, optimizer, logger, device, save_path):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            final_features, element_logits, v_features, tibi_loss, gate = model(visual_features, visual_features, seq_len)

            # Compute MIL loss
            ce_loss = KMXMILL_individual(element_logits, seq_len, videolabels, device, loss_type='CE')

            # Combine losses
            # npcr_weight = 5.0  # Adjust weights as needed
            tibi_weight = 5.0
            total_loss = 5*ce_loss + tibi_weight * tibi_loss
            # total_loss = 5*ce_loss
            # Optimization step
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
